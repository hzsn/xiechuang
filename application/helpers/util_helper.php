...
if (!function_exists('pre_next_format')) {
    ...
    function pre_next_format(array $array, $id){
        ...
        if (count($array) == 1) {
            ...
            if ($array[0]['id'] > $id) {
                $_class = 'text-right';
                $_title = '<span>下一篇:</span><a href="'.site_url('/article/'.$array[0]["id"]).'">'.$array[0]['title'].'</a>';
                ...
                $_class = 'text-right';
                $_title = '<span>上一篇:</span><a href="'.site_url('/article/'.$array[0]["id"]).'">'.$array[0]['title'].'</a>';
            ...
            foreach ($array  as $key => $value) {
                if ($value['id'] > $id) {
                    $_class = 'text-right';
                    $_title = '<span>下一篇:</span><a href="'.site_url('/article/'.$value["id"]).'">'.$value['title'].'</a>';
                }elseif ($value['id'] < $id) {
                    $_class = 'text-left';
                    $_title = '<span>上一篇:</span><a href="'.site_url('/article/'.$value["id"]).'">'.$value['title'].'</a>';
                }
...
if (!function_exists('news_format')) {
     ...
     * @return [type]        [description]
     */
    function news_format(array $array, $dir_path){
        $re_arr['code'] =  0;
        if (!is_array($array) || empty($array)) {
            $re_arr['code'] = 1;
            $re_arr['msg'] = '暂无资讯';
        ...
        $div = '<div class="col-md-8"><div class="xc-news-i-box xc-news-large animated bounceInLeft"><div class="col-md-6 xc-news-i-img"  style="background-image: url('.$dir_path.$array[0]['item_img'].');"><a class="xc-news-i-title text-right" href="'.site_url('/article/'.$array[0]["id"]).'">'.$array[0]['title'].'</a><span class="xc-news-i-date" style="right: 0">'.explode(' ', $array[0]['create_time'])[0].'</span></div><div class="col-md-6 xc-height100"><div class="xc-news-i-content text-indent-2em">'.$array[0]['summary'].'</div></div></div>';
        $len = count($array);
        if ($len == 1) {
            $div .= '</div>';
        }
        if ($len == 2) {
            $div .= '<div class="xc-news-i-box xc-news-large animated bounceInLeft"><div class="col-md-6 xc-height100"><div class="xc-news-i-content text-indent-2em">'.$array[1]['summary'].'</div></div><div class="col-md-6 xc-news-i-img"  style="background-image: url('.$dir_path.$array[1]['item_img'].');"><a class="xc-news-i-title text-right" href="'.site_url('/article/'.$array[1]["id"]).'">'.$array[1]['title'].'</a><span class="xc-news-i-date" style="left: 0">'.explode(' ', $array[1]['create_time'])[0].'</span></div></div>';
            $div .= '</div>';
        }
        if ($len > 2) {
             $div .= '<div class="xc-news-i-box xc-news-large animated bounceInLeft"><div class="col-md-6 xc-height100"><div class="xc-news-i-content text-indent-2em">'.$array[1]['summary'].'</div></div><div class="col-md-6 xc-news-i-img"  style="background-image: url('.$dir_path.$array[1]['item_img'].');"><a class="xc-news-i-title text-right" href="'.site_url('/article/'.$array[1]["id"]).'">'.$array[1]['title'].'</a><span class="xc-news-i-date" style="left: 0">'.explode(' ', $array[1]['create_time'])[0].'</span></div></div>';
            $div .= '</div>';
           $div .= '<div class="col-md-4 animated fadeInRightShort" style="padding-left: 0px;"><div class="xc-news-i-box"><div class="xc-news-i-img" style="background-image: url('.$dir_path.$array[2]['item_img'].');height: 157.5px;"><a class="xc-news-i-title text-right" href="'.site_url('/article/'.$array[2]["id"]).'">'.$array[2]['title'].'</a><span class="xc-news-i-date" style="right: 0">'.explode(' ', $array[2]['create_time'])[0].'</span>
                </div><div class="xc-height100 xc-news-small">
                   <div class="xc-news-i-content text-indent-2em">'.$array[2]['summary'].'</div></div></div></div>';
        }
        $re_arr['item'] = $div;
        return $re_arr;
    }
